#!/bin/bash
# TGmonitor Phase 3 & 4 Deployment Script
# Run on VPS: cd /var/app/tgmonitor && bash deploy_phase4.sh
#
# Phase 3: Gemini AI analysis, alerts, daily journals
# Phase 4: Next.js web dashboard

set -e
cd /var/app/tgmonitor

echo "=== TGmonitor Phase 3 & 4 Deployment ==="
echo ""
...
echo "[3/5] Rebuilding backend with AI analysis..."
docker compose build backend
echo "  Backend build complete."

# Step 4: Build frontend
echo "[4/5] Building frontend..."
docker compose build frontend || (cd frontend && npm install && npm run build && cd ..)
...
curl -sf http://localhost:3000 && echo "Frontend: OK" || echo "Frontend: FAILED (may need a moment)"
...
echo "  - 30-day screenshot cleanup: runs every 24h"
echo ""
echo "IMPORTANT: Set required env vars in .env if not already set:"
echo "  GEMINI_API_KEY=your_gemini_key"
echo "  ADMIN_PASSWORD=your_dashboard_password"
